...
export default function VotingModerationPage() {
  ...
  const getBadgeStyle = (type: ModerationFlag["flagType"]) => {
    switch (type) {
      case "rapid_clicking":
        return { label: "Rapid Click Spam (<1s/beat)", color: "text-red-400 bg-red-500/10 border-red-500/30" };
      case "multi_account_ip":
        return { label: "IP/Device Collusion", color: "text-purple-400 bg-purple-500/10 border-purple-500/30" };
      case "incomplete_votes":
        return { label: "Below Minimum Threshold", color: "text-amber-400 bg-amber-500/10 border-amber-500/30" };
      default:
        return { label: "Outlier Rating", color: "text-blue-400 bg-blue-500/10 border-blue-500/30" };
    }
  };
    ...
    <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-surface-border pb-6">
        <div>
          ...
          <p className="text-xs text-zinc-400 mt-1">
            Human-in-the-loop fraud prevention for Beat Battle #5 community preselections.
          </p>
        </div>

        {/* Metric Cards */}
        <div className="flex items-center gap-3">
          <div className="bg-surface-card border border-surface-border rounded-xl px-4 py-2 text-center">
      ...
      <div className="space-y-4">
        {filteredFlags.length === 0 ? (
          <div className="bg-surface-card border border-surface-border rounded-2xl p-12 text-center text-zinc-400">
          ...
          filteredFlags.map((flag) => {
            ...
            return (
              <div
                key={flag.id}
                className="bg-surface-card border border-surface-border rounded-2xl p-6 space-y-4 hover:border-zinc-700 transition-all shadow-md"
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div className="flex items-center gap-3">
                    <span className={`px-2.5 py-1 rounded-lg text-xs font-bold border font-mono ${badge.color}`}>
                      {badge.label}
                    </span>
                ...
                <div className="bg-surface-subtle border border-surface-border rounded-xl p-4 space-y-2">
                  ...
                  <p className="text-xs text-zinc-300 leading-relaxed pt-2 border-t border-surface-border/50">
                ...
                {flag.status === "pending" && (
                  <div className="flex items-center justify-end gap-3 pt-2">
                    <button
                      ...
                      className="px-4 py-2 rounded-xl bg-surface-subtle hover:bg-emerald-500/20 text-zinc-300 hover:text-emerald-300 text-xs font-bold border border-surface-border transition-colors flex items-center gap-1.5"
                    >
