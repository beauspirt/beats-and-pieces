...
import { ShieldCheck, CheckCircle2, ArrowRight, UserCheck, Sparkles } from "lucide-react";
...
export default function SignInPage() {
  ...
  const handleEmailSubmit = (e: React.FormEvent) => {
    ...
    const res = loginWithEmail(inputEmail);
    if (res.isMatchedProducer) {
      setMatchStatus({
        matched: true,
  ...
  const handleQuickLogin = (producerId: string) => {
    const prod = sampleProducers[producerId];
    if (prod) {
  ...
  return (
    <div className="min-h-[75vh] flex flex-col items-center justify-center text-center px-4 animate-in fade-in duration-300">
      ...
      <p className="text-sm text-[#8E8E93] max-w-md mb-8 leading-relaxed">
        Sign in to access beat battles, vote on submissions, and manage your artist profile.
      </p>

      {/* Auth Buttons */}
      <div className="flex flex-col sm:flex-row items-center gap-3.5 w-full max-w-xs mb-10">
        {/* Google OAuth Button */}
        <button
          onClick={handleGoogleClick}
          className="w-full py-3.5 px-5 rounded-xl bg-white hover:bg-zinc-100 text-black text-xs font-bold transition-all shadow-lg active:scale-95 flex items-center justify-center gap-2.5 cursor-pointer"
        >
        ...
        <button
          onClick={() => {
            loginWithUser("usr-nerub");
            router.push("/profile");
          }}
          className="w-full py-3.5 px-5 rounded-xl bg-[#5865F2] hover:bg-[#4752C4] text-white text-xs font-bold transition-all shadow-lg active:scale-95 flex items-center justify-center gap-2.5 cursor-pointer"
        >
      ...
      <div className="w-full max-w-2xl bg-[#181818] border border-white/5 rounded-2xl p-5 sm:p-6 text-left space-y-4 shadow-xl">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-xs sm:text-sm font-bold text-white uppercase tracking-wider">
              Quick Sign-In (Legacy Beatmaker Accounts)
            </h3>
          </div>
        ...
        <p className="text-xs text-[#999999] leading-relaxed">
          Log in with any legacy beatmaker email to immediately access that artist&apos;s verified profile showcase:
        </p>
      ...
      {showEmailModal && (
        ...
        >
          <div
            ...
            className="bg-[#181818] rounded-2xl max-w-md w-full p-6 sm:p-8 space-y-6 shadow-2xl relative border border-white/10 cursor-default"
          >
            <div className="flex items-center justify-between">
            ...
            {matchStatus ? (
              <div className="text-center py-4 space-y-3 animate-in zoom-in-95">
                ...
                <div>
                  <h4 className="text-base font-bold text-white">
                    Welcome back, {matchStatus.nickname}!
                  </h4>
                  <p className="text-xs text-[#999999] mt-1 font-mono">
                    {matchStatus.email}
                  </p>
                  {matchStatus.matched && (
                    <span className="inline-flex items-center gap-1 text-[11px] text-[#7B61FF] font-semibold mt-2 bg-[#7B61FF]/10 px-3 py-1 rounded-full">
                      <Sparkles className="w-3 h-3" />
                      <span>Linked to Verified Battle History</span>
                    </span>
                  )}
                </div>
              </div>
              ...
              <form onSubmit={handleEmailSubmit} className="space-y-4 text-left">
                <p className="text-xs text-[#A0A0A0] leading-relaxed">
                  Enter your Google account email to sign in and automatically link your historical beatmaker profile:
                </p>

                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-[#D1D1D1]">Google Email</label>
                  <input
                    type="email"
                    required
                    autoFocus
                    placeholder="e.g. vulpeanumihaivalentin@gmail.com"
                    value={inputEmail}
                    onChange={(e) => setInputEmail(e.target.value)}
                    className="w-full bg-[#121212] border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:ring-1 focus:ring-[#7B61FF]"
                  />
                </div>
                ...
                <div className="flex items-center justify-end gap-3 pt-2">
                  <button
                    type="button"
                    onClick={() => setShowEmailModal(false)}
                    className="px-5 py-2.5 rounded-xl bg-[#222222] hover:bg-[#2A2A2A] text-xs font-semibold text-[#A0A0A0] hover:text-white transition-colors cursor-pointer"
                  >
                    Cancel
                  </button>
                  <button
